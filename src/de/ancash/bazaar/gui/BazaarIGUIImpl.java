...
public class BazaarIGUIImpl extends AbstractBazaarIGUI{

	
	
	public BazaarIGUIImpl(BazaarInventoryClassManager clazzManager) {
		super(clazzManager);
	}
	...
	public Map<String, String> getPlaceholders(int cat, int sub, int subsub, boolean topSellOffers, int tSOcnt, boolean topBuyOrders, int tBOcnt) {
		...
		if(cat > 0) {
			...
			List<SelfBalancingBST> allBuyOrders = subsub != -1 ? Arrays.asList(category.getBuyOrders(sub, subsub)): category.getAllBuyOrders(sub);
			List<SelfBalancingBST> allSellOffers = subsub != -1 ? Arrays.asList(category.getSellOffers(sub, subsub)) : category.getAllSellOffers(sub);
	...
	public double getHighestBuyOrderPrice() {
		...
		return max == null ? Category.getCategory(getCurrentCategory()).getEmptyPrice(getCurrentSub(), getCurrentSubSub()) : max.getKey();
	}
	...
	public double getLowestBuyOrderPrice() {
		...
		return min == null ? Category.getCategory(getCurrentCategory()).getEmptyPrice(getCurrentSub(), getCurrentSubSub()) : min.getKey();
	}
