...
import de.ancash.bazaar.gui.inventory.AbstractBazaarManageEnquiriesInv;
...
public class BazaarManageEnquiriesInv extends AbstractBazaarManageEnquiriesInv{
	
	public BazaarManageEnquiriesInv(BazaarInventoryClassManager clazzManager) {
		super(clazzManager);
	}
	...
	@Override
	public synchronized void collect(AbstractBazaarIGUI igui, double price, int category, int sub, int subsub, String uuid,	EnquiryType type) {
		...
		case SELL_OFFER:
			...
			player.sendMessage("§6Bazaar! §7Claimed §6" + coins + " coins §7from selling §a" + claimable + "§7x " + Category.getCategory(category).getSubSub()[sub - 1][subsub - 1].getItemMeta().getDisplayName() + " §7at §6" + price + " §7each!");
		...
		case BUY_ORDER:
			...
			player.sendMessage("§6Bazaar! §7Claimed §a" + adding + "§7x " + Category.getCategory(category).getSubSub()[sub- 1][subsub - 1].getItemMeta().getDisplayName() + " §7worth §6" + (adding * price) + " coins §7bought for §6" + price + " §7each!");
			break;
		default:
			break;
		}
