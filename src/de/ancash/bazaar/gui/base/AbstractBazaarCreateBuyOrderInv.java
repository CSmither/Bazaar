...
public abstract class AbstractBazaarCreateBuyOrderInv {
	...
	public final void openSelectPrice(AbstractBazaarIGUI igui) {
		if(Bukkit.isPrimaryThread()) 
			igui.plugin.submit(new BazaarRunnable(igui.getId()) {
				...
				public void run() {
					...
					new BukkitRunnable() {
						...
						@Override
						public void run() {
							openSelectPriceSync(igui, placeholders.getFirst(), placeholders.getSecond(), placeholders.getThird());
						}
					}.runTaskLater(igui.plugin, 1);
				}
			});
	...
	private final void openConfirmInventorySync(AbstractBazaarIGUI igui, double pricePerUnit) {
		...
		placeholders.put(BazaarPlaceholder.DISPLAY_NAME, Category.getCategory(igui.currentCategory).getSubSubShow(igui.currentSub, igui.currentSubSub).getItemMeta().getDisplayName());
